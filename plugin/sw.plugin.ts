...
const replaceImports = async (index: number, entry: string, code: string): Promise<string> => {
  ...
  for (const node of ast.program.body) {
    ...
    const virtual = `${raw[0]}${await revealFile(entry, raw.slice(1, -1))}?${VIRTUAL_IMPORT}:${index}${raw[0]}`;
...
const loadModule = async (root: string, id: string): Promise<string> => {
  ...
  try {
    pkg = JSON.parse(await fs.readFile(resolve(root, 'node_modules', id, 'package.json'), 'utf-8'));
  } catch (e) {
...
const checkSWFile = (name: string) => {
  const reg = new RegExp(`\\?${VIRTUAL_IMPORT}:[0-9]$`)
...
export const nativeSW = ({ entries }: Options): Plugin[] => {
  ...
  return [{
    ...
    async configResolved(config: ResolvedConfig) {
      conf = config;
      await Promise.all(
        entries.map(async ({ src, dist, genVersion }) => versions[dist] = genVersion ? await genVersion() : randomBytes(20).toString('hex').slice(0, 8)),
      );
    },
    buildStart() {
      entries.map(({ src, dist }) => this.emitFile({ type: 'chunk', id: src, fileName: dist }));
    },
